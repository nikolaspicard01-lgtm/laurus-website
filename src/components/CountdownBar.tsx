  ...
  return (
    <div className="relative gradient-coral text-white text-center py-3 px-4 text-[13px] sm:text-[14px] font-bold overflow-hidden">
      ...
      <div className="relative flex items-center justify-center gap-3 sm:gap-5 flex-wrap">
        <span>
          {t.text} <span className="underline decoration-2 underline-offset-2">{t.discount}</span> {t.marchOnly}
        </span>
        <div className="flex items-center gap-1.5">
          {[
            ...
            <div key={u.l} className="bg-white/15 backdrop-blur-sm rounded-lg px-2.5 py-1 min-w-[40px] text-center">
              <span className="text-[15px] font-black tabular-nums">{String(u.v).padStart(2, "0")}</span>
      ...
      <button onClick={() => setVisible(false)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/60 hover:text-white p-1" aria-label="Close">
